...
void wifi_cap_set_from_capability_information(uint8_t *bitmap, uint8_t *ie, size_t ie_len)
{
...
void wifi_cap_set_from_ht_capabilities_info(uint8_t *bitmap, uint8_t *ie, size_t ie_len)
{
	if (!bitmap || !ie || ie_len < 2)
		return;
	if (!!(ie[0] & bit(0)))
...
void wifi_cap_set_from_vht_capabilities_info(uint8_t *bitmap, uint8_t *ie, size_t ie_len)
{
	int vht_mpdu = 0;
	int vht_rx_stbc = 0;
	int vht_chw = 0;
...
void wifi_cap_set_from_extended_capabilities(uint8_t *bitmap, uint8_t *ie, size_t ie_len)
{
...
void wifi_cap_set_from_rm_enabled_capabilities(uint8_t *bitmap, uint8_t *ie, size_t ie_len)
{
...
void wifi_cap_set_from_fast_bss_transition(uint8_t *bitmap, uint8_t *ie, size_t ie_len)
{
	if (!bitmap || !ie || ie_len < 3)
		return;
...
void wifi_cap_set_from_he(uint8_t *bmp, uint8_t *ie, size_t ielen)
{
	UNUSED(ielen);
...
void wifi_cap_set_from_ie_vendor(uint8_t *bitmap, uint8_t *ie, size_t ielen)
{
	...
	char wmm_ie[] = { 0x00, 0x50, 0xf2, 0x02 };
...
{
	char wmm_oui[] = { 0x00, 0x50, 0xf2, 0x02 };
...
static int is_wpa_ie(uint8_t *ie, size_t len)
{
	if (ie[0] == IE_RSN)
		return 1;
