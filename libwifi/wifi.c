...
int wifi_get_scan_results(const char *ifname, struct wifi_bss *bsss, int *nr)
{
	...
	ENTER();
	if (drv && drv->get_scan_results)
		ret = drv->get_scan_results(ifname, bsss, nr);
						...
						struct wifi_bss_detail *b)
{
	...
	ENTER();
	if (drv && drv->get_bss_scan_result)
		ret = drv->get_bss_scan_result(ifname, bssid, b);
...
int wifi_get_caps(const char *ifname, struct wifi_caps *caps)
{
	...
	ENTER();
	if (drv && drv->iface.get_caps)
		ret = drv->iface.get_caps(ifname, caps);
...
int wifi_radio_get_caps(const char *name, struct wifi_caps *caps)
{
	...
	ENTER();
	if (drv && drv->radio.get_caps)
		ret = drv->radio.get_caps(name, caps);
...
int wifi_get_supp_stds(const char *name, uint8_t *std)
{
	...
	ENTER();
	if (drv && drv->radio.get_supp_stds)
		ret = drv->radio.get_supp_stds(name, std);
...
int wifi_get_bssid(const char *ifname, uint8_t *bssid)
{
	...
	ENTER();
	if (drv && drv->get_bssid)
		ret = drv->get_bssid(ifname, bssid);
...
int wifi_get_channel(const char *ifname, uint32_t *channel, enum wifi_bw *bw)
{
	...
	ENTER();
	if (drv && drv->get_channel)
		ret = drv->get_channel(ifname, channel, bw);
			...
			uint8_t *in, int ilen, uint8_t *out, int *olen)
{
	...
	ENTER();
	if (drv && drv->vendor_cmd)
		ret = drv->vendor_cmd(ifname, vid, subcmd, in, ilen, out, olen);
...
int wifi_subscribe_frame(const char *ifname, uint8_t type, uint8_t stype)
{
	...
	ENTER();
	if (drv && drv->iface.subscribe_frame)
		ret = drv->iface.subscribe_frame(ifname, type, stype);
...
int wifi_unsubscribe_frame(const char *ifname, uint8_t type, uint8_t stype)
{
	...
	ENTER();
	if (drv && drv->iface.unsubscribe_frame)
		ret = drv->iface.unsubscribe_frame(ifname, type, stype);
...
int wifi_set_4addr(const char *ifname, bool enable)
{
	...
	ENTER();
	if (drv && drv->set_4addr)
		ret = drv->set_4addr(ifname, enable);
...
int wifi_get_4addr(const char *ifname, bool *enabled)
{
	...
	ENTER();
	if (drv && drv->get_4addr)
		ret = drv->get_4addr(ifname, enabled);

	EXIT(ret);
	return ret;
}

int wifi_get_4addr_parent(const char *ifname, const char *parent)
{
	const struct wifi_driver *drv = get_wifi_driver(ifname);
	int ret = -ENOTSUP;

	ENTER();
	if (drv && drv->get_4addr_parent)
		ret = drv->get_4addr_parent(ifname, parent);
...
int wifi_link_measure(const char *ifname, uint8_t *sta)
{
	...
	ENTER();
	if (drv && drv->link_measure)
		ret = drv->link_measure(ifname, sta);
...
int wifi_set_channel(const char *ifname, uint32_t channel, enum wifi_bw bw)
{
	...
	ENTER();
	if (drv && drv->set_channel)
		ret = drv->set_channel(ifname, channel, bw);
...
int wifi_chan_switch(const char *ifname, struct chan_switch_param *param)
{
	...
	ENTER();
	if (drv && drv->chan_switch)
		ret = drv->chan_switch(ifname, param);
...
int wifi_mbo_disallow_assoc(const char *ifname, uint8_t reason)
{
	...
	ENTER();
	if (drv && drv->mbo_disallow_assoc)
		ret = drv->mbo_disallow_assoc(ifname, reason);
...
int wifi_ap_set_state(const char *ifname, bool up)
{
	...
	ENTER();
	if (drv && drv->ap_set_state)
		ret = drv->ap_set_state(ifname, up);
					...
					enum wifi_bw bw)
{
	...
	ENTER();
	if (drv && drv->get_supp_channels)
		ret = drv->get_supp_channels(ifname, chlist, num,
					     cc, band, bw);
	else
...
int wifi_get_oper_stds(const char *name, uint8_t *std)
{
	...
	ENTER();
	if (drv && drv->get_oper_stds)
		ret = drv->get_oper_stds(name, std);
...
int wifi_get_country(const char *name, char *alpha2)
{
	...
	ENTER();
	if (drv && drv->get_country)
		ret = drv->get_country(name, alpha2);
...
int wifi_get_countrylist(const char *name, char *cc, int *num)
{
	...
	ENTER();
	if (drv && drv->get_countrylist)
		ret = drv->get_countrylist(name, cc, num);
...
int wifi_get_oper_band(const char *name, enum wifi_band *band)
{
	...
	ENTER();
	if (drv && drv->get_oper_band)
		ret = drv->get_oper_band(name, band);
...
int wifi_get_supp_band(const char *name, uint32_t *bands)
{
	...
	ENTER();
	if (drv && drv->get_supp_band)
		ret = drv->get_supp_band(name, bands);
...
int wifi_get_mode(const char *ifname, enum wifi_mode *mode)
{
	...
	ENTER();
	if (drv && drv->iface.get_mode)
		ret = drv->iface.get_mode(ifname, mode);
...
int wifi_get_security(const char *ifname, uint32_t *auth, uint32_t *enc)
{
	...
	ENTER();
	if (drv && drv->iface.get_security)
		ret = drv->iface.get_security(ifname, auth, enc);
...
int wifi_get_maxrate(const char *ifname, unsigned long *rate)
{
	...
	ENTER();
	if (drv && drv->get_maxrate)
		ret = drv->get_maxrate(ifname, rate);
						...
						struct wifi_opclass *o)
{
	...
	ENTER();
	if (drv && drv->get_supp_opclass)
		ret = drv->get_supp_opclass(name, num_opclass, o);
	else
...
int wifi_get_curr_opclass(const char *name, struct wifi_opclass *o)
{
	...
	ENTER();
	if (drv && drv->get_curr_opclass)
		ret = drv->get_curr_opclass(name, o);
	else
...
int wifi_get_bandwidth(const char *name, enum wifi_bw *bw)
{
	...
	ENTER();
	if (drv && drv->get_bandwidth)
		ret = drv->get_bandwidth(name, bw);
...
int wifi_get_supp_bandwidths(const char *name, uint32_t *bws)
{
	...
	ENTER();
	if (drv && drv->get_supp_bandwidths)
		ret = drv->get_supp_bandwidths(name, bws);
...
int wifi_get_noise(const char *ifname, int *noise)
{
	...
	ENTER();
	if (drv && drv->get_noise)
		ret = drv->get_noise(ifname, noise);
...
int wifi_radio_get_stats(const char *ifname, struct wifi_radio_stats *s)
{
	...
	ENTER();
	if (drv && drv->radio.get_stats)
		ret = drv->radio.get_stats(ifname, s);
...
int wifi_radio_info(const char *ifname, struct wifi_radio *info)
{
	...
	ENTER();
	if (drv && drv->radio.info)
		ret = drv->radio.info(ifname, info);
...
int wifi_radio_get_hwaddr(const char *ifname, uint8_t *hwaddr)
{
	...
	ENTER();
	if (drv && drv->radio.get_hwaddr)
		ret = drv->radio.get_hwaddr(ifname, hwaddr);
...
int wifi_ap_info(const char *ifname, struct wifi_ap *info)
{
	...
	ENTER();
	if (drv && drv->iface.ap_info)
		ret = drv->iface.ap_info(ifname, info);
...
int wifi_ap_get_stats(const char *ifname, struct wifi_ap_stats *s)
{
	...
	ENTER();
	if (drv && drv->iface.get_stats)
		ret = drv->iface.get_stats(ifname, s);
...
int wifi_get_beacon_ies(const char *ifname, uint8_t *ies, int *len)
{
	...
	ENTER();
	if (drv && drv->get_beacon_ies)
		ret = drv->get_beacon_ies(ifname, ies, len);
...
int wifi_get_assoclist(const char *ifname, uint8_t *stas, int *num)
{
	...
	ENTER();
	if (drv && drv->get_assoclist)
		ret = drv->get_assoclist(ifname, stas, num);
...
int wifi_get_sta_info(const char *ifname, uint8_t *addr, struct wifi_sta *info)
{
	...
	ENTER();
	if (drv && drv->get_sta_info)
		ret = drv->get_sta_info(ifname, addr, info);
...
int wifi_radio_get_param(const char *name, const char *param, int *len, void *val)
{
	...
	ENTER();
	if (drv && drv->radio.get_param)
		ret = drv->radio.get_param(name, param, len, val);
...
int wifi_add_iface(const char *name, enum wifi_mode m, char *argv[])
{
	...
	ENTER();
	if (drv && drv->add_iface)
		ret = drv->add_iface(name, m, argv);
...
int wifi_del_iface(const char *name, const char *ifname)
{
	...
	ENTER();
	if (drv && drv->del_iface)
		ret = drv->del_iface(name, ifname);
...
int wifi_list_iface(const char *name, struct iface_entry *iface, int *num)
{
	...
	ENTER();
	if (drv && drv->list_iface)
		ret = drv->list_iface(name, iface, num);
...
int wifi_channels_info(const char *name, struct chan_entry *channel, int *num)
{
	...
	ENTER();
	if (drv && drv->channels_info)
		ret = drv->channels_info(name, channel, num);
		   ...
		   enum wifi_cac_method method)
{
	...
	ENTER();
	if (drv && drv->start_cac)
		ret = drv->start_cac(name, channel, bw, method);
				 ...
				 int *num)
{
	...
	ENTER();
	if (drv && drv->get_opclass_preferences)
		ret = drv->get_opclass_preferences(name, opclass, num);
...
int wifi_simulate_radar(const char *name, struct wifi_radar_args *radar)
{
	...
	ENTER();
	if (drv && drv->simulate_radar)
		ret = drv->simulate_radar(name, radar);
...
int wifi_disconnect_sta(const char *ifname, uint8_t *sta, uint16_t reason)
{
	...
	ENTER();
	if (drv && drv->disconnect_sta)
		ret = drv->disconnect_sta(ifname, sta, reason);
...
int wifi_monitor_sta(const char *ifname, uint8_t *sta, struct wifi_monsta_config *cfg)
{
	...
	ENTER();
	if (drv && drv->monitor_sta)
		ret = drv->monitor_sta(ifname, sta, cfg);
...
int wifi_get_monitor_sta(const char *ifname, uint8_t *sta, struct wifi_monsta *mon)
{
	...
	ENTER();
	if (drv && drv->get_monitor_sta)
		ret = drv->get_monitor_sta(ifname, sta, mon);
...
int wifi_get_monitor_stas(const char *ifname, struct wifi_monsta *stas, int *num_stas)
{
	...
	ENTER();
	if (drv && drv->get_monitor_stas)
		ret = drv->get_monitor_stas(ifname, stas, num_stas);
...
int wifi_probe_sta(const char *ifname, uint8_t *sta)
{
	...
	ENTER();
	if (drv && drv->iface.probe_sta)
		ret = drv->iface.probe_sta(ifname, sta);
...
int wifi_restrict_sta(const char *ifname, uint8_t *sta, int enable)
{
	...
	ENTER();
	if (drv && drv->restrict_sta)
		ret = drv->restrict_sta(ifname, sta, enable);
...
int wifi_get_neighbor_list(const char *ifname, struct nbr *nbrs, int *nr)
{
	...
	ENTER();
	if (drv && drv->iface.get_neighbor_list)
		ret = drv->iface.get_neighbor_list(ifname, nbrs, nr);
...
int wifi_add_neighbor(const char *ifname, struct nbr nbr)
{
	...
	ENTER();
	if (drv && drv->iface.add_neighbor)
		ret = drv->iface.add_neighbor(ifname, nbr);
...
int wifi_del_neighbor(const char *ifname, unsigned char *bssid)
{
	...
	ENTER();
	if (drv && drv->iface.del_neighbor)
		ret = drv->iface.del_neighbor(ifname, bssid);
				...
				struct wifi_beacon_req *param, size_t param_sz)
{
	...
	ENTER();
	if (drv && drv->iface.req_beacon_report)
		ret = drv->iface.req_beacon_report(ifname, sta,
					param, param_sz);
				...
				struct sta_nbr *snbrs, int *nr)
{
	...
	ENTER();
	if (drv && drv->iface.get_beacon_report)
		ret = drv->iface.get_beacon_report(ifname, sta, snbrs, nr);
		...
		int bsss_nr, unsigned char *bsss, unsigned int tmo)
{
	...
	ENTER();
	if (drv && drv->iface.req_bss_transition)
		ret = drv->iface.req_bss_transition(ifname, sta, bsss_nr,
						    bsss, tmo);
				...
				struct nbr *bsss, struct wifi_btmreq *b)
{
	...
	ENTER();
	if (drv && drv->iface.req_btm)
		ret = drv->iface.req_btm(ifname, sta, bsss_nr, bsss, b);
			...
			uint8_t *r1khid)
{
	...
	ENTER();
	if (drv && drv->iface.get_11rkeys)
		ret = drv->iface.get_11rkeys(ifname, sta, r1khid);
...
int wifi_set_11rkeys(const char *ifname, struct fbt_keys *fk)
{
	...
	ENTER();
	if (drv && drv->iface.set_11rkeys)
		ret = drv->iface.set_11rkeys(ifname, fk);
...
int wifi_add_vendor_ie(const char *ifname, struct vendor_iereq *req)
{
	...
	ENTER();
	if (drv && drv->iface.add_vendor_ie)
		ret = drv->iface.add_vendor_ie(ifname, req);
...
int wifi_del_vendor_ie(const char *ifname, struct vendor_iereq *req)
{
	...
	ENTER();
	if (drv && drv->iface.del_vendor_ie)
		ret = drv->iface.del_vendor_ie(ifname, req);
...
int wifi_get_vendor_ies(const char *ifname, struct vendor_ie *ies, int *num_ies)
{
	...
	ENTER();
	if (drv && drv->iface.get_vendor_ies)
		ret = drv->iface.get_vendor_ies(ifname, ies, num_ies);
...
int wifi_sta_info(const char *ifname, struct wifi_sta *sta)
{
	...
	ENTER();
	if (drv && drv->iface.sta_info)
		ret = drv->iface.sta_info(ifname, sta);
...
int wifi_sta_get_stats(const char *ifname, struct wifi_sta_stats *s)
{
	...
	ENTER();
	if (drv && drv->iface.sta_get_stats)
		ret = drv->iface.sta_get_stats(ifname, s);
...
int wifi_sta_get_ap_info(const char *ifname, struct wifi_bss *info)
{
	...
	ENTER();
	if (drv && drv->iface.sta_get_ap_info)
		ret = drv->iface.sta_get_ap_info(ifname, info);
...
int wifi_sta_disconnect_ap(const char *ifname, uint32_t reason)
{
	...
	ENTER();
	if (drv && drv->iface.sta_disconnect_ap)
		ret = drv->iface.sta_disconnect_ap(ifname, reason);
...
int wifi_register_event(const char *ifname, struct event_struct *ev, void **handle)
{
	...
	if (drv) {
		if (drv->register_event)
			ret = drv->register_event(ifname, ev, handle);
		else
...
int wifi_unregister_event(const char *ifname, void *handle)
{
	...
	drv = get_wifi_driver(ifname);
	if (drv) {
		if (drv->unregister_event)
			ret = drv->unregister_event(ifname, handle);
		else
...
int wifi_recv_event(const char *ifname, void *handle)
{
	...
	if (drv) {
		if (drv->recv_event)
			ret = drv->recv_event(ifname, handle);
		else
...
int wifi_start_wps(const char *ifname, struct wps_param wps)
{
	...
	ENTER();
	if (drv && drv->iface.start_wps)
		ret = drv->iface.start_wps(ifname, wps);
...
int wifi_stop_wps(const char *ifname)
{
	...
	ENTER();
	if (drv && drv->iface.stop_wps)
		ret = drv->iface.stop_wps(ifname);
...
int wifi_get_wps_status(const char *ifname, enum wps_status *s)
{
	...
	ENTER();
	if (drv && drv->iface.get_wps_status)
		ret = drv->iface.get_wps_status(ifname, s);
...
int wifi_get_wps_pin(const char *ifname, unsigned long *pin)
{
	...
	ENTER();
	if (drv && drv->iface.get_wps_pin)
		ret = drv->iface.get_wps_pin(ifname, pin);
...
int wifi_set_wps_pin(const char *ifname, unsigned long pin)
{
	...
	ENTER();
	if (drv && drv->iface.set_wps_pin)
		ret = drv->iface.set_wps_pin(ifname, pin);
...
int wifi_get_wps_device_info(const char *ifname, struct wps_device *info)
{
	...
	ENTER();
	if (drv && drv->iface.get_wps_device_info)
		ret = drv->iface.get_wps_device_info(ifname, info);
