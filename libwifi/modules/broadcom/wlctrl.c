...
static int wl_vendor(struct wl_arg *arg)
{
	...
	if (arg->iovar) {
		...
		snprintf(in + req->offset, sizeof(in) - req->offset, "%s", arg->iovar);
		inlen += len;
	}
...
static int _brcm_get_ap_wmm_params(const char *ifname, uint8_t *p,
				   struct wifi_ap_wmm_ac a[])
{
	a[0].ac = (p[0] & 0x60) >> 5;
	a[0].aifsn = p[0] & 0x0f;
		    ...
		    enum wifi_cac_method method)
{
	...
	if (method != WIFI_CAC_CONTINUOUS && method != WIFI_CAC_MIMO_REDUCED) {
		libwifi_dbg("[%s] %s unsupported cac method\n", ifname, __func__);
		return -1;
	}
	...
	/* stunt ap move because this cac is for pre-clearing only */
	res = Cmd(buf, sizeof(buf),
...
static int bcmwl_radio_get_mcs_index_per_nss(uint16_t *mcsbuf, uint32_t len, uint32_t *mcs_idx)
{
	uint8_t mcs_tmp;
	uint8_t mcs_max = 0;
	int i, j;
...
static int bcmwl_radio_get_mcs_index(uint8_t *mcsbuf, uint32_t len, uint32_t *mcs_idx)
{
	uint8_t mcs_tmp = 0;
	int i, j;
...
static int bcmwl_event_handle(const char *ifname, bcm_event_t *event, int size, char *buf, size_t buf_size)
{
	...
	if (datalen > 0)
		snprintf(buf + strlen(buf), buf_size, ", \"raw\":\"%s\"", raw);
...
int bcmwl_iface_get_4addr_parent(const char *ifname, const char *parent)
{
	int ret;
